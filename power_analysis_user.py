...
cleandatafile='E:\\datawang\\HuaweiData-20170615\\3. Result\\Software\\1. Preprocessing\\0. Clean Data_users.xls'
...
current_user =table.cell(2,1).value
...
for i in range(2,nrows):
	if table.cell(i,1).value==current_user and table.cell(i,0).value==current_app:
		...
		pasheet.write(row,1,current_user)
		pasheet.write(row,2,sum_bt/(sum_ft+sum_bt))
		if sum_ft!= 0:
			pasheet.write(row,3,sum_fp/sum_ft)
		else:
			pasheet.write(row,3,'NAN')
		if sum_bt!= 0:
			pasheet.write(row,4,sum_bp/sum_bt)
		else:
			pasheet.write(row,4,'NAN')

		if(sum_ft==0 and sum_bt!=0):
			pasheet.write(row,5,'NAN')
			pasheet.write(row,6,sum_wu/sum_bt)
			pasheet.write(row,7,'NAN')
			pasheet.write(row,8,sum_br/sum_bt)
			pasheet.write(row,9,'NAN')
			pasheet.write(row,10,sum_gs/sum_bt)
			pasheet.write(row,11,'NAN')
			pasheet.write(row,12,sum_wl/sum_bt)
			pasheet.write(row,13,'NAN')
			pasheet.write(row,14,sum_ss/sum_bt)
			pasheet.write(row,15,'NAN')
			pasheet.write(row,16,sum_gu/sum_bt)
		elif(sum_ft!=0 and sum_bt==0):
			pasheet.write(row,5,sum_wu/sum_ft)
			pasheet.write(row,6,'NAN')
			pasheet.write(row,7,sum_br/sum_ft)
			pasheet.write(row,8,'NAN')
			pasheet.write(row,9,sum_gs/sum_ft)
			pasheet.write(row,10,'NAN')
			pasheet.write(row,11,sum_wl/sum_ft)
			pasheet.write(row,12,'NAN')
			pasheet.write(row,13,sum_ss/sum_ft)
			pasheet.write(row,14,'NAN')
			pasheet.write(row,15,sum_gu/sum_ft)
			pasheet.write(row,16,'NAN')
		else:
			...
			pasheet.write(row,5,x1_s/sum_a)
			pasheet.write(row,6,y1_s/sum_a)
			pasheet.write(row,7,x2_s/sum_a)
			pasheet.write(row,8,y2_s/sum_a)
			pasheet.write(row,9,x3_s/sum_a)
			pasheet.write(row,10,y3_s/sum_a)
			pasheet.write(row,11,x4_s/sum_a)
			pasheet.write(row,12,y4_s/sum_a)
			pasheet.write(row,13,x5_s/sum_a)
			pasheet.write(row,14,y5_s/sum_a)
			pasheet.write(row,15,x6_s/sum_a)
			pasheet.write(row,16,y6_s/sum_a)

		row=row+1
		if table.cell(i,1).value!=current_user and table.cell(i,0).value==current_app:
			current_user=table.cell(i,1).value
		else:
			current_user=table.cell(i,1).value
...
pasheet.write(row,7,x2_s/sum_a)
pasheet.write(row,8,y2_s/sum_a)
pasheet.write(row,9,x2_s/sum_a)
pasheet.write(row,10,y2_s/sum_a)
pasheet.write(row,11,x2_s/sum_a)
pasheet.write(row,12,y2_s/sum_a)
pasheet.write(row,13,x2_s/sum_a)
pasheet.write(row,14,y2_s/sum_a)
pasheet.write(row,15,x2_s/sum_a)
pasheet.write(row,16,y2_s/sum_a)
