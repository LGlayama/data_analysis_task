...
current_users=table.cell(2,1).value
...
for i in range(2,nrows):
	if table.cell(i,1).value==current_users and table.cell(i,0).value==current_app:
		...
		bisheet.write(row,1,current_users)

		bisheet.write(row,2,sum_ft/sum_a)
		bisheet.write(row,3,sum_bt/sum_a)
		bisheet.write(row,4,sum_f/sum_a)
		bisheet.write(row,5,sum_b/sum_a)
		bisheet.write(row,6,sum_fb/sum_a)
		row=row+1
		if table.cell(i,1).value!=current_users and table.cell(i,0).value==current_app:
			current_users=table.cell(i,1).value
		else:
			current_users=table.cell(i,1).value
